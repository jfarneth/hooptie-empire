 ...
 */
export function openNegotiation(rng: RngState, anchor: number, overpricing: number): Negotiation {
  ...
  const room = normalish(rng, CFG.roomMean, CFG.roomSpread, 0, 1);
  ...
  if (chance(rng, 0.3)) tellIndex += chance(rng, 0.5) ? 1 : -1;
...
export function resolveCounter(
  ...
  if (neg.countersMade >= CFG.maxPlayerCounters) {
    neg.status = 'walked';
    return { kind: 'walked' };
  }
...
export function countersRemaining(neg: Negotiation): number {
  return Math.max(0, CFG.maxPlayerCounters - neg.countersMade);
}

/** Where the sales desk pitches its one standing counter. */
export function deskCounter(neg: Negotiation): number {
  const increment = roundingIncrement(neg.anchor);
  const target = neg.currentOffer + (neg.anchor - neg.currentOffer) * CFG.deskCounterFraction;
